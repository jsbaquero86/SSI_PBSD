...
def modfactors(X1,X2,X5,X6,X7,X10,X13):
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    alphaVals = np.array([round(alpha10th,3),round(alpha50th,3),round(alpha90th,3),round(alphaGBRT,3)]).reshape(-1,1)
    alphaCdVals = np.array([round(alphaCd10thX5,3),round(alphaCd50thX5,3),round(alphaCd90thX5,3),
                            round(alphaCd10thX10,3),round(alphaCd50thX10,3),round(alphaCd90thX10,3),round(alphaCdGBRT,3)]).reshape(-1,1)

    alphaDF = pd.DataFrame(data=alphaVals,columns=['Estimated $\\alpha$'],index=['$\\alpha_{10^{th}}$','$\\alpha_{10^{th}}$','$\\alpha_{50^{th}}$','$\\alpha_{GBRT}$'])
    alphaCdDF = pd.DataFrame(data=alphaCdVals, columns=['Estimated $\\alpha_{C_d}$'], index=['$\\alpha(X_5)_{C_d(10^{th})}$','$\\alpha(X_5)_{C_d(50^{th})}$',
                                                                                            '$\\alpha(X_5)_{C_d(90^{th})}$','$\\alpha(X_{10})_{C_d(10^{th})}$',
                                                                                            '$\\alpha(X_{10})_{C_d(50^{th})}$','$\\alpha(X_{10})_{C_d(90^{th})}$',
                                                                                            '$\\alpha_{C_d(GBRT)}$'])
    display(alphaDF)
    display(alphaCdDF)
    
    return alphaDF, alphaCdDF